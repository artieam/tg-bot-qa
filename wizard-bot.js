...
const kvApi = (method, params) => {
    return fetch('<DB_URL>' + method, params);
};
...
const saveUserData = async (ctx) => {
    let userData = ctx.from;
    userData.result = ctx.session.result;
    userData.step = ctx.session.cnt;
    await saveUser(userData);
};

const getUser = (user) => {
    return kvApi('user/' + user.id, {
        method: 'GET',
        headers: {
            'Content-Type': 'application/json',
        },
...
const getUserResult = async (ctx) => {
    ...
    try {
        ...
        let res = 0;
        if (!userD.error) {
            return JSON.parse(userD.value);
        } else {
            return {result: -1, step: -1};
        }
    } catch (e) {
        return {result: -1, step: -1};
...
const subjectStep = (ctx, checkVal, errorMessage, nextSubject, doNextStep) => {
    ...
    if (r === false) {
        ctx.session.cnt = -1;
        return ctx.reply("(Пустой текст)");
    }
    const d = checkCmd(ctx);
    if (d === false) {
        return ctx.reply("(Базовые команды отключены)");
    }
    if (ctx.message.text !== checkVal && ctx.message.text !== "/end" && ctx.message.text !== "/ls") {
        if (doNextStep == true) {
            ctx.session.cnt++;
    ...
    if (ctx.message.text === checkVal) {
        if (ctx.session.cnt == 9 || ctx.session.cnt == 10) {
            ctx.session.result += 2;
        } else {
            ctx.session.result++;
        }
        ctx.session.cnt++;
        return ctx.reply(nextSubject);
    }
};
...
bot.start(async (ctx) => {
    ...
    return ctx.reply(
        ...
        }!\n\nПредлагаю решить 10 задач: за 2 сложные даю по 2 балла, за 8 простых – по одному. Правила очень простые. Удачи!\n\nДля начала напиши "start"`
    );
});
...
bot.command("res", async (ctx) => {
    ...
    return ctx.reply(`RES: ` + res.result + ` STEP: ` + res.step);
});
bot.command("ls", async (ctx) => {
    const res = await getUserResult(ctx);
    ctx.session.cnt = res.step;
    ctx.session.result = res.result;
    return ctx.reply("сессия = " + (ctx.session.cnt ?? "--") + "\n" + "результат = " + (ctx.session.result ?? "--"))
});
bot.command("end", async (ctx) => {
    ctx.session.cnt = -1;
    ctx.session.result = -1;
    const e = await saveUserData(ctx);
    return ctx.reply("Завершаю тест");
});
